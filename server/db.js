...
`);
    ...
    // email_settings was designed to hold exactly one row, but nothing
    // enforced that - a save-vs-save race (check-then-act INSERT/UPDATE)
    // could silently create a second row, and every read used
    // `LIMIT 1` with no ORDER BY, so which row (and which recipients) came
    // back was non-deterministic. Add a real singleton constraint so this
    // can't happen again, and an atomic upsert (see emailRoutes.js) closes
    ...
    await client.query(`
      ...
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns
          WHERE table_name = 'email_settings' AND column_name = 'singleton'
        ) THEN
          ALTER TABLE email_settings ADD COLUMN singleton BOOLEAN NOT NULL DEFAULT true;
        END IF;
      END $$;
    `);

    // Collapse any pre-existing duplicate rows before the unique constraint
    // is added below (keeps whichever row was updated most recently)
    await client.query(`
      DO $$
      DECLARE
        keeper_id INTEGER;
      BEGIN
        IF (SELECT COUNT(*) FROM email_settings) > 1 THEN
          SELECT id INTO keeper_id FROM email_settings
          ORDER BY updated_at DESC NULLS LAST, id DESC LIMIT 1;
          DELETE FROM email_settings WHERE id <> keeper_id;
        END IF;
      END $$;
    `);
    ...
    await client.query(`
      ...
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM pg_constraint WHERE conname = 'email_settings_singleton_key'
        ) THEN
          ALTER TABLE email_settings ADD CONSTRAINT email_settings_singleton_key UNIQUE (singleton);
        END IF;
      END $$;
    `);

    // Seed a default row if the table is completely empty
    await client.query(`
    ...
    // Automatic sending: up to two times a day, both optional. Stored as
    // TIME (no date/timezone) - the scheduler always interprets them as
    // Asia/Beirut local time regardless of what timezone the server itself
    // runs in (see server/emailScheduler.js).
    await client.query(`
      DO $$
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns
          WHERE table_name = 'email_settings' AND column_name = 'scheduled_sending_enabled'
        ) THEN
          ALTER TABLE email_settings ADD COLUMN scheduled_sending_enabled BOOLEAN NOT NULL DEFAULT false;
        END IF;
      END $$;
    `);
    ...
    await client.query(`
      ...
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns
          WHERE table_name = 'email_settings' AND column_name = 'send_time_1'
        ) THEN
          ALTER TABLE email_settings ADD COLUMN send_time_1 TIME;
        END IF;
      END $$;
    `);
    ...
    await client.query(`
      ...
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns
          WHERE table_name = 'email_settings' AND column_name = 'send_time_2'
        ) THEN
          ALTER TABLE email_settings ADD COLUMN send_time_2 TIME;
        END IF;
      END $$;
    `);
