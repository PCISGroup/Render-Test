import { Resend } from 'resend';
import { format, isWeekend } from 'date-fns';
import { query } from './db.js';

const escapeHtml = (value) => {
  if (value === null || value === undefined) return '';
  return String(value)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;');
};

// "Jane" / "Jane and John" / "Jane, John and Alex" - same grammar as the
// on-page overlap notice
const formatNameList = (names) => {
  if (names.length <= 1) return names[0] || '';
  if (names.length === 2) return `${names[0]} and ${names[1]}`;
  return `${names.slice(0, -1).join(', ')} and ${names[names.length - 1]}`;
};

// Postgres hands timestamp/date columns back through JSON_BUILD_OBJECT as
// plain strings - parse defensively the same way the rest of the app does.
const parseTimestamp = (value) => {
  if (!value) return null;
  const date = new Date(value);
  return isNaN(date.getTime()) ? null : date;
};

// Turns one grouped schedule row into what the templates need to render:
// a label ("Client X (Installation)") plus an optional colored detail line
// for postponed/completed/cancelled state.
const buildStatusEntry = (row) => {
  const { base, types, state, cancellationReason, completedAt, postponedDate } = row;
  const label = types ? `${base} (${types})` : base;

  let detail = null;
  let stateClass = '';

  switch (state) {
    case 'Cancelled':
      detail = cancellationReason ? `Cancelled — ${cancellationReason}` : 'Cancelled';
      stateClass = 'state-cancelled';
      break;
    case 'TBA':
      detail = 'Postponed — date TBA';
      stateClass = 'state-postponed';
      break;
    case 'PostponedFrom': {
      const original = parseTimestamp(postponedDate);
      detail = original ? `Postponed from ${format(original, 'MMM d, yyyy')}` : 'Postponed';
      stateClass = 'state-postponed';
      break;
    }
    case 'Completed': {
      const completed = parseTimestamp(completedAt);
      detail = completed ? `Completed at ${format(completed, 'h:mm a')}` : 'Completed';
      stateClass = 'state-completed';
      break;
    }
    default:
      break;
  }

  return { label, detail, stateClass };
};

class EmailService {
  constructor() {
    this.resend = new Resend(process.env.RESEND_API_KEY);
    this.fromEmail = process.env.FROM_EMAIL || 'PCIS Group <info@pcis.group>';

    console.log('✅ Resend email service initialized');
    console.log('📧 Sender:', this.fromEmail);
  }

  // Send email immediately (on-demand)
  async sendEmailNow(settings, scheduleData, overlaps = []) {
    console.log('🚀 Sending immediate email via Resend');

    try {
      if (!scheduleData || scheduleData.length === 0) {
        throw new Error('No schedule data to send');
      }

      const today = new Date();

      let filteredScheduleData = scheduleData;
      if (!settings.include_weekends && isWeekend(today)) {
        console.log('Weekend detected and include_weekends is false -> skipping email');
        throw new Error('Today is a weekend and weekend emails are disabled');
      }

      // Generate HTML (same as before)
      const emailHtml = this.generateEmailHTML(filteredScheduleData, today, overlaps);

      // Generate plain text (same as before)
      const textContent = this.generatePlainText(filteredScheduleData, today, overlaps);

      console.log(`📧 Sending to: ${settings.recipients.join(', ')}`);

      // Send via Resend
      const { data, error } = await this.resend.emails.send({
        from: this.fromEmail,
        to: settings.recipients,
        subject: `PCIS Group Daily Schedule - ${format(today, 'MMMM d, yyyy')}`,
        html: emailHtml,
        text: textContent,
      });

      if (error) {
        console.error('❌ Resend error:', error);
        throw new Error(`Resend error: ${error.message}`);
      }

      console.log('✅ Email sent successfully via Resend. Message ID:', data?.id);
      return { messageId: data?.id };

    } catch (error) {
      console.error('❌ Email failed:', error);
      throw error;
    }
  }

  async getTodaysScheduleFromDB(date) {
  const dateStr = format(date, 'yyyy-MM-dd');

  try {
    const result = await query(`
      WITH schedule_data AS (
        SELECT
          e.id as employee_id,
          e.name,
          e.ext,
          CASE
            WHEN es.client_id IS NOT NULL THEN
              COALESCE(c.name, 'Client ' || es.client_id::TEXT)
            WHEN (s.label ILIKE '%with%') AND es.with_employee_id IS NOT NULL THEN
              'With ' || COALESCE(we.name, 'Unknown')
            WHEN (s.label ILIKE '%with%') AND es.with_employee_id IS NULL THEN
              'With ...'
            ELSE
              COALESCE(s.label, 'Status ' || es.status_id::TEXT)
          END as base_status,
          COALESCE(st.type_name, '') as type_name,
          es.id as schedule_id,
          ss.state_name,
          es.postponed_date,
          es.completed_at,
          cr.reason as cancellation_reason,
          CASE
            WHEN LOWER(COALESCE(ss.state_name, '')) = 'cancelled' THEN 'Cancelled'
            WHEN LOWER(COALESCE(ss.state_name, '')) = 'postponed'
                 AND (es.postponed_date IS NULL OR es.postponed_date::TEXT = '') THEN 'TBA'
            WHEN LOWER(COALESCE(ss.state_name, '')) = 'postponed' THEN 'PostponedFrom'
            WHEN LOWER(COALESCE(ss.state_name, '')) = 'completed' THEN 'Completed'
            ELSE ''
          END as state_prefix
        FROM employee_schedule es
        JOIN employees e ON es.employee_id = e.id
        LEFT JOIN statuses s ON es.status_id = s.id
        LEFT JOIN clients c ON es.client_id = c.id
        LEFT JOIN employees we ON es.with_employee_id = we.id
        LEFT JOIN schedule_types st ON es.schedule_type_id = st.id
        LEFT JOIN schedule_states ss ON es.schedule_state_id = ss.id
        LEFT JOIN cancellation_reasons cr ON es.cancellation_reason_id = cr.id
        WHERE es.date = $1
      ),
      grouped_data AS (
        SELECT
          employee_id,
          name,
          ext,
          base_status,
          CASE
            WHEN MAX(CASE WHEN state_prefix = 'Cancelled' THEN 1 ELSE 0 END) = 1 THEN 'Cancelled'
            WHEN MAX(CASE WHEN state_prefix = 'PostponedFrom' THEN 1 ELSE 0 END) = 1 THEN 'PostponedFrom'
            WHEN MAX(CASE WHEN state_prefix = 'TBA' THEN 1 ELSE 0 END) = 1 THEN 'TBA'
            WHEN MAX(CASE WHEN state_prefix = 'Completed' THEN 1 ELSE 0 END) = 1 THEN 'Completed'
            ELSE ''
          END as state_prefix,
          MAX(cancellation_reason) as cancellation_reason,
          MAX(completed_at) as completed_at,
          MAX(postponed_date) as postponed_date,
          STRING_AGG(DISTINCT type_name, ' - ' ORDER BY type_name) FILTER (WHERE type_name != '') as types,
          MIN(schedule_id) as display_order
        FROM schedule_data
        GROUP BY employee_id, name, ext, base_status
      )
      SELECT
        name,
        ext,
        ARRAY_AGG(
          JSON_BUILD_OBJECT(
            'base', base_status,
            'types', types,
            'state', state_prefix,
            'cancellationReason', cancellation_reason,
            'completedAt', completed_at,
            'postponedDate', postponed_date
          )
          ORDER BY display_order
        ) as status_details
      FROM grouped_data
      GROUP BY name, ext
      ORDER BY MIN(display_order)
    `, [dateStr]);

    const scheduleData = result.rows.map(row => ({
      name: row.name,
      ext: row.ext || '',
      statuses: (row.status_details || []).map(buildStatusEntry)
    }));

    console.log(`📊 FINAL: ${scheduleData.length} employees in schedule (including Cancelled/TBA/Completed)`);

    return scheduleData;

  } catch (error) {
    console.error('❌ Error fetching schedule data:', error);
    return [];
  }
}

  // Clients with 2+ different employees assigned today - same overlap logic
  // the schedule page's notification bell already surfaces
  async getTodaysOverlaps(date) {
    const dateStr = format(date, 'yyyy-MM-dd');

    try {
      const result = await query(`
        SELECT c.name as client_name, ARRAY_AGG(DISTINCT e.name ORDER BY e.name) as employee_names
        FROM employee_schedule es
        JOIN employees e ON es.employee_id = e.id
        JOIN clients c ON es.client_id = c.id
        WHERE es.date = $1 AND es.client_id IS NOT NULL
        GROUP BY c.id, c.name
        HAVING COUNT(DISTINCT es.employee_id) >= 2
        ORDER BY c.name
      `, [dateStr]);

      return result.rows.map(row => ({
        clientName: row.client_name,
        employeeNames: row.employee_names
      }));
    } catch (error) {
      console.error('❌ Error fetching overlap data:', error);
      return [];
    }
  }

  generateEmailHTML(scheduleData, date, overlaps = []) {
    return `
    <!DOCTYPE html>
    <html>
      <head>
        <style>
          body {
            font-family: Arial, sans-serif;
            line-height: 1.5;
            color: #333;
            margin: 0;
            padding: 0;
            background-color: #ffffff;
          }
          .container {
            max-width: 700px;
            margin: 0 auto;
            padding: 20px;
          }
          .header {
            text-align: center;
            padding: 20px 0;
            border-bottom: 2px solid #e5e7eb;
            margin-bottom: 20px;
          }
          .header h1 {
            font-size: 24px;
            font-weight: 600;
            margin: 0 0 5px 0;
            color: #111827;
          }
          .header h2 {
            font-size: 16px;
            font-weight: 400;
            margin: 0;
            color: #6b7280;
          }
          .schedule-table {
            width: 100%;
            border-collapse: collapse;
            margin: 20px 0;
            font-size: 14px;
          }
          .schedule-table th {
            background-color: #f9fafb;
            font-weight: 600;
            padding: 12px 16px;
            border-bottom: 2px solid #e5e7eb;
            text-align: left;
            color: #374151;
          }
          .schedule-table td {
            padding: 12px 16px;
            border-bottom: 1px solid #e5e7eb;
            text-align: left;
            vertical-align: top;
          }
          .schedule-table tr:last-child td {
            border-bottom: none;
          }
          .status-entry {
            margin-bottom: 6px;
          }
          .status-entry:last-child {
            margin-bottom: 0;
          }
          .status-label {
            font-size: 13px;
            color: #374151;
            font-weight: 600;
          }
          .status-detail {
            font-size: 12px;
            margin-top: 1px;
          }
          .state-completed { color: #059669; }
          .state-cancelled { color: #dc2626; }
          .state-postponed { color: #d97706; }
          .overlap-section {
            background: #fffbeb;
            border: 1px solid #fde68a;
            border-radius: 6px;
            padding: 12px 16px;
            margin: 20px 0;
          }
          .overlap-note {
            font-size: 13px;
            color: #92400e;
            margin: 0;
          }
          .overlap-note:not(:last-child) {
            margin-bottom: 6px;
          }
          .overlap-note strong {
            color: #7c3e0a;
          }
          .footer {
            text-align: center;
            margin-top: 30px;
            color: #9ca3af;
            font-size: 12px;
            padding: 16px 0 4px 0;
          }
          .no-data {
            text-align: center;
            padding: 40px 20px;
            color: #6b7280;
            font-style: italic;
            background-color: #f9fafb;
            border-radius: 6px;
          }
          .weekend-notice {
            background: #f9fafb;
            border: 1px solid #e5e7eb;
            border-radius: 6px;
            padding: 12px;
            margin: 15px 0;
            text-align: center;
            color: #6b7280;
            font-size: 14px;
          }
        </style>
      </head>
      <body>
        <div class="container">
          <div class="header">
            <h1>Electra Engineering</h1>
            <h2>Daily Schedule - ${format(date, 'MMMM d, yyyy')}</h2>
          </div>

          ${isWeekend(date) ? `
            <div class="weekend-notice">
              <strong>Weekend Schedule</strong>
            </div>
          ` : ''}

          ${scheduleData.length > 0 ? `
            <table class="schedule-table">
              <thead>
                <tr>
                  <th>Employee</th>
                  <th>Extension</th>
                  <th>Status</th>
                </tr>
              </thead>
              <tbody>
                ${scheduleData.map(emp => `
                  <tr>
    <td><strong>${escapeHtml(emp.name)}</strong></td>
    <td>${escapeHtml(emp.ext)}</td>
    <td>
      ${emp.statuses.map(s => `
        <div class="status-entry">
          <div class="status-label">${escapeHtml(s.label)}</div>
          ${s.detail ? `<div class="status-detail ${s.stateClass}">${escapeHtml(s.detail)}</div>` : ''}
        </div>
      `).join('')}
    </td>
  </tr>
                `).join('')}
              </tbody>
            </table>
          ` : `
            <div class="no-data">
              <p>No schedule data available for ${format(date, 'MMMM d, yyyy')}</p>
            </div>
          `}

          ${overlaps.length > 0 ? `
            <div class="overlap-section">
              ${overlaps.map(o => `
                <p class="overlap-note">
                  ⚠ Today: <strong>${escapeHtml(formatNameList(o.employeeNames))}</strong> are going to <strong>${escapeHtml(o.clientName)}</strong>.
                </p>
              `).join('')}
            </div>
          ` : ''}

          <div class="footer">
            <p>Generated automatically by PCIS Scheduler System</p>
          </div>
        </div>
      </body>
    </html>
  `;
  }

  generatePlainText(scheduleData, date, overlaps = []) {
    if (scheduleData.length === 0) {
      return `Electra Engineering Daily Schedule - ${format(date, 'MMMM d, yyyy')}\n\nNo schedule data available for today.\n\nGenerated automatically by Electra Engineering Schedule System`;
    }

    let text = `Electra Engineering Daily Schedule - ${format(date, 'MMMM d, yyyy')}\n\n`;
    if (isWeekend(date)) text += "WEEKEND SCHEDULE\n\n";

    scheduleData.forEach(emp => {
      const statusLines = emp.statuses
        .map(s => (s.detail ? `${s.label} (${s.detail})` : s.label))
        .join(' - ');
      text += `${emp.name} (Ext: ${emp.ext}) - ${statusLines}\n`;
    });

    if (overlaps.length > 0) {
      text += '\n';
      overlaps.forEach(o => {
        text += `Today: ${formatNameList(o.employeeNames)} are going to ${o.clientName}.\n`;
      });
    }

    text += '\nGenerated automatically by PCIS Scheduler System';
    return text;
  }
}

export default EmailService;