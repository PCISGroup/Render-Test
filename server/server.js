...
const ALLOWED_EMAILS = ['info@pcis.group', 'se.admin@pcis.group', "mohamad.fattouh@bioteckpharm.health", "mohamad.fattouh@electra.health", "sales@bioteckpharm.health"];
  ...
  try {
    ...
    if (extension) {
      const empResult = await query(
        'SELECT id, name, email, ext FROM employees WHERE ext = $1',
        [extension.trim()]
      );
      ...
      if (empResult.rows.length === 1) {
        ...
        // Validate that the Microsoft email matches the employee's email in database
        if (employeeEmail && userEmail === employeeEmail) {
          req.session.userEmail = userEmail;
          ...
          // Email doesn't match the employee in database
          return res.status(403).json({ 
            error: 'Email mismatch - your Microsoft email does not match this extension in the system' 
          });
        }
      }
    }
...
ORDER BY es.date
    ...
    const scheduleStates = result.rows.map(row => {
      // Return the date as-is from the database (no timezone conversion)
      // The frontend sends dates as 'yyyy-MM-dd' and we store them as-is
      ...
      console.log('🔍 State for display:', { statusId: row.status_identifier, date: dateStr, state: row.state_name, rawDate: row.date });
      return stateObj;
    });
    
    console.log('🔍 Total schedule states to return:', scheduleStates.length);
...
app.post('/api/schedule-state', requireSession, async (req, res) => {
  ...
  try {
    ...
    // Only the 'completed' state carries a completion timestamp - any other
    // state (including re-completing later) clears/sets it accordingly.
    const completedAt = stateName === 'completed' ? new Date() : null;
    ...
    if (stateName === 'postponed') {
      ...
      if (isTBA) {
        ...
        const updateQuery = `
          ...
          SET schedule_state_id = $${paramIndex}, postponed_date = NULL, completed_at = $${paramIndex + 1}, updated_at = CURRENT_TIMESTAMP
          WHERE ${whereClause}
          RETURNING *
        `;

        params.push(scheduleStateId, completedAt);
      ...
      if (result.rows.length === 0) {
        ...
        if (statusId.startsWith('status-')) {
          ...
          insertQuery = `
            INSERT INTO employee_schedule
            (employee_id, date, status_id, schedule_state_id, completed_at)
            VALUES ($1, $2, $3, $4, $5)
            RETURNING *
          `;
          insertParams.splice(2, 0, statusNum);
          ...
          if (statusId.includes('_type-')) {
            ...
            insertQuery = `
              INSERT INTO employee_schedule
              (employee_id, date, client_id, schedule_type_id, schedule_state_id, completed_at)
              VALUES ($1, $2, $3, $4, $5, $6)
              RETURNING *
            `;
            insertParams.splice(2, 0, clientNum, typeNum);
            ...
            insertQuery = `
              INSERT INTO employee_schedule
              (employee_id, date, client_id, schedule_state_id, completed_at)
              VALUES ($1, $2, $3, $4, $5)
              RETURNING *
            `;
            insertParams.splice(2, 0, clientNum);
          ...
          if (parts.length >= 3) {
            ...
            if (statusPart.startsWith('status-')) {
              ...
              insertQuery = `
                INSERT INTO employee_schedule
                (employee_id, date, with_employee_id, status_id, schedule_state_id, completed_at)
                VALUES ($1, $2, $3, $4, $5, $6)
                RETURNING *
              `;
              insertParams.splice(2, 0, withEmployeeId, statusNum);
      ...
      try {
        if (typeof statusId === 'string' && statusId.startsWith('client-') && statusId.includes('_type-')) {
          ...
          if (fbResult.rows.length > 0) {
            ...
            console.log('Fallback also found no matching entries for client', clientNum);
          }
        } else {
          console.log('No fallback applicable for statusId:', statusId);
        }
      } catch (fbErr) {
...
app.delete('/api/schedule-state', requireSession, async (req, res) => {
  try {
    ...
    if (statusId.startsWith('status-')) {
      ...
      if (statusId.includes('_type-')) {
        // Typed client: clear state on the exact client+type row, keep the row itself
        // (removing the schedule entry entirely is handled separately by POST /api/schedule)
        const [clientPart, typePart] = statusId.split('_type-');
        const clientNum = parseInt(clientPart.replace('client-', ''), 10);
        const typeNum = parseInt(typePart, 10);
      ...
      return res.status(400).json({
        success: false,
        error: `Unrecognized statusId format: ${statusId}`
      });
    }

    // Always just clear the state marker - never delete the schedule entry itself.
    // Entry removal is handled by POST /api/schedule's reconciliation logic.
    const query = `
      UPDATE employee_schedule
      SET schedule_state_id = NULL, postponed_date = NULL, completed_at = NULL, updated_at = CURRENT_TIMESTAMP
      WHERE ${whereClause}
      RETURNING *
    `;
    const result = await pool.query(query, params);
    ...
    res.json({
      success: true,
      message: 'Schedule state cleared'
    });
