...
export default function Analytics() {
  ...
  const fetchScheduleStates = useCallback(async () => {
    ...
    try {
      ...
      // Build a date window that also covers future schedules.
      // This is required for postponed items moved to upcoming dates.
      const endDate = new Date();
      const startDate = new Date();
      startDate.setDate(startDate.getDate() - 90);
      endDate.setDate(endDate.getDate() + 365);

      // If schedule data has wider bounds, extend to cover them.
      if (scheduleData && typeof scheduleData === 'object') {
        Object.values(scheduleData).forEach((employeeSchedules) => {
          if (!employeeSchedules || typeof employeeSchedules !== 'object') return;

          Object.keys(employeeSchedules).forEach((dateKey) => {
            if (!/^\d{4}-\d{2}-\d{2}$/.test(dateKey)) return;
            const dateObj = new Date(`${dateKey}T00:00:00`);
            if (Number.isNaN(dateObj.getTime())) return;

            if (dateObj < startDate) {
              startDate.setTime(dateObj.getTime());
            }
            if (dateObj > endDate) {
              endDate.setTime(dateObj.getTime());
            }
          });
        });
      }
  ...
  }, [employeesArray, employeesArray.length, scheduleData]);
