...
// "Jane" / "Jane and John" / "Jane, John and Alex"
...
export default function SchedulePage() {
  ...
  // Notification data: today only - which clients have 2+ employees assigned
  // today (same overlap the schedule grid already colors, surfaced up front
  // so it's visible without opening anything).
  const todaySharedClientOverlaps = useMemo(() => {
    const todayStr = format(new Date(), "yyyy-MM-dd");
    const clientEmployees = {}; // baseClientId -> employees assigned (deduped)

    employees.forEach((emp) => {
      const statuses = schedules[emp.id]?.[todayStr] || [];
      const clientsForEmployee = new Set();
      statuses.forEach((statusId) => {
        if (typeof statusId === 'string' && statusId.startsWith('client-')) {
          clientsForEmployee.add(statusId.split('_type-')[0]);
        }
      });
      clientsForEmployee.forEach((baseClientId) => {
        if (!clientEmployees[baseClientId]) clientEmployees[baseClientId] = [];
        clientEmployees[baseClientId].push(emp);
      });
    });

    return Object.entries(clientEmployees)
      .filter(([, emps]) => emps.length >= 2)
      .map(([baseClientId, emps]) => ({
        clientId: baseClientId,
        clientName: statusConfigs.find((s) => s.id === baseClientId)?.name || 'Client',
        employees: emps
      }));
  }, [employees, schedules, statusConfigs]);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (result.success && result.scheduleStates) {
            console.log(`✅ API returned ${result.scheduleStates.length} states for employee ${employeeId}`);
            result.scheduleStates.forEach(state => {
              const dateStr = state.date; 
              const key = `${employeeId}_${dateStr}`;
              
              console.log('📥 Processing state:', { key, statusId: state.status_id, stateName: state.state_name, date: dateStr });
              ...
              if (state.status_id && state.state_name) {
                ...
                console.log('✅ Stored state for baseId:', baseStatusId, 'in key:', key);
              } else {
                console.warn('⚠️ Skipped state - missing status_id or state_name:', { status_id: state.status_id, state_name: state.state_name });
              }
            });
          } else {
            console.warn(`⚠️ API returned invalid data for employee ${employeeId}`);
          }
        }
      } catch (err) {
  ...
  if (selectedEmployee) {
    ...
    if (typeof statusId === 'string' && statusId.includes('_type-')) {
      ...
      if (dayStatuses.includes(statusId)) {
        ...
        if (!otherTypesExist) {
          // Removing the last type shouldn't drop the client assignment or its state -
          // keep it as a bare client with no type (matches employeeDashboard's toggleStatus).
          // statusStates is keyed by baseClientId already, and the backend's POST /api/schedule
          // reconciliation preserves schedule_state_id when it strips the type off the row,
          // so nothing needs to be deleted here.
          newStatuses.push(baseClientId);
          console.log('✅ No other types exist - keeping client with no type, state preserved:', baseClientId);
        } else {
...
const removeStatus = useCallback(async (employeeId, dateStr, statusId, scheduleUpdate = null) => {
  ...
  if (shouldRemoveState) {
    console.log("Removing state for key:", stateKeyToRemove);
    
    setStatusStates(prev => {
      const newStates = { ...prev };
      const key = `${employeeId}_${dateStr}`;
      
      console.log("Looking for state at key:", key, "for:", stateKeyToRemove);
    ...
    // Also delete from backend - use the FULL statusId (not the stripped baseClientId)
    // so a typed client (e.g. "client-1_type-2") matches its actual DB row, which has
    // schedule_type_id set. stateKeyToRemove is only correct for the local statusStates key.
    if (stateKeyToRemove) {
      await deleteStateFromBackend(employeeId, dateStr, statusId);
    }
  } else {
  ...
  const handleExport = useCallback(() => {
    ...
    // Helper to deduplicate: skip base client if typed version exists
    const getUniqueStatusNames = (statusIds) => {
      const names = [];
      const seenBaseClients = new Set();
      
      statusIds.forEach(statusId => {
        // Check if this is a typed client (client-1_type-2)
        if (typeof statusId === 'string' && statusId.includes('_type-')) {
          const baseClientId = statusId.split('_type-')[0];
          seenBaseClients.add(baseClientId);
          names.push(getStatusName(statusId));
        }
        // Check if this is a base client and we haven't seen a typed version
        else if (typeof statusId === 'string' && statusId.startsWith('client-')) {
          if (!seenBaseClients.has(statusId)) {
            names.push(getStatusName(statusId));
          }
        }
        // For other statuses (status-X, with_X) always include
        else {
          names.push(getStatusName(statusId));
        }
      });
    ...
    if (exportRange === "day" || nonEmptyDates.length === 1) {
      ...
      employees.forEach(employee => {
        ...
        if (statuses.length > 0) {
          const statusNames = getUniqueStatusNames(statuses).join("-");
    ...
    const filename = exportRange === "day" || nonEmptyDates.length === 1 
      ? `Electra_Schedule_${format(nonEmptyDates[0], 'yyyy-MM-dd')}.csv`
      : `Electra_Schedule_${format(nonEmptyDates[0], 'yyyy-MM-dd')}_to_${format(nonEmptyDates[nonEmptyDates.length - 1], 'yyyy-MM-dd')}.csv`;
    ...
    <div className="schedule-page">
      <div className="page-header">
        ...
        <div className="header-actions">
          <NotificationBell overlaps={todaySharedClientOverlaps} />
          <button
      ...
      <div className="controls">
        ...
        <div className="view-switch">
          <button
            className={`tab-btn ${viewType === "today" ? "active" : ""}`}
            onClick={() => {
      ...
      <h2 className="date-title">
        {viewType === "week"
          ? `Week of ${format(dateRange[0], "MMM d, yyyy")}`
          : format(currentDate, "EEEE, MMM d, yyyy")}
      </h2>

      {todaySharedClientOverlaps.length > 0 && (
        <div className="overlap-banner">
          {todaySharedClientOverlaps.map((overlap) => (
            <div key={overlap.clientId} className="overlap-banner-line">
              <Users size={15} className="overlap-banner-icon" />
              <span className="overlap-banner-text">
                Today: <strong>{formatNameList(overlap.employees.map((emp) => emp.name))}</strong> are going to{' '}
                <strong>{overlap.clientName}</strong>
              </span>
            </div>
          ))}
        </div>
              ...
              <div className="export-section">
                ...
                <div className="export-options">
                  {[
                    {
                      value: "current",
                      label: `Current View (${viewType === "week" ? "Week" : "Today"})`,
                    },
