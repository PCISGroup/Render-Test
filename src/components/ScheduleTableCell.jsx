...
// Postgres can hand back timestamps in a few different shapes depending on the
// column type and driver path, so try a few parse strategies before giving up.
...
const formatTimestamp = (value) => {
  ...
  try {
    ...
    if (!date) return value; // Return raw value if we can't parse it
...
// Just the time (e.g. "9:00 AM") - used where the date is already implied
// by context (a cell on that day's column), so repeating it would be noise.
...
const ScheduleTableCell = ({
  ...
  useEffect(() => {
    ...
    console.log('🔄 ScheduleTableCell useEffect - Loading localStates', { 
      key, 
      statusStatesAtKey: statusStates?.[key],
      hasStatusStates: !!statusStates
    });

    if (statusStates && statusStates[key]) {
      console.log('✅ Found statusStates for key:', key, statusStates[key]);
      Object.keys(statusStates[key]).forEach((statusId) => {
        ...
        if (!updatedLocalStates[baseId]) {
          ...
          console.log('✅ Set localState for baseId:', baseId, updatedLocalStates[baseId]);
        }
      });
    } else {
      console.warn('⚠️ No statusStates found for key:', key);
    }

    console.log('🔄 Updated localStates:', updatedLocalStates);
  ...
  // Colors the WHOLE card when a client is shared with another employee
  // today - light enough that the dark card text stays readable, saturated
  // enough to actually stand out against the plain white cards around it.
  const generateClientColor = (baseId) => {
  const match = baseId.match(/client-(\d+)/);
  if (!match) return '#e5e7eb';
  const id = parseInt(match[1]);

  // Golden ratio method - ensures maximum distinction
  const hue = (id * 0.618033988749895 * 360) % 360;
  const saturation = 62;
  const lightness = 87;
...
const API_BASE_URL = import.meta.env.VITE_API_URL || 'http://localhost:5001';
  ...
  const saveState = useCallback(async (statusId, stateName, postponedDate = null, isTBA = false, reason = null, note = null) => {
    ...
    try {
      ...
      if (stateName === 'postponed') {
        ...
        // Optimistic completion timestamp - backend recalculates the source of
        // truth on the next refresh, this just avoids a blank flash until then
  ...
  const clearState = useCallback(async (statusId) => {
    ...
    try {
      ...
      // Restore the state if error - must key by baseId, same as everywhere else
      // localStates/statusStates are read (e.g. line ~882 `localStates[baseId]`)
      if (currentState) {
        setLocalStates(prev => ({
          ...prev,
          [baseId]: currentState
        ...
        if (onStatusStateChange) {
          onStatusStateChange(employee.id, dateStr, baseId, currentState);
        }
      }
  ...
  // Show "Postponed from [date]" in display
  ...
  const getRegionInitials = (statusId) => {
    if (typeof statusId === 'string' && statusId.startsWith('client-')) {
      ...
      return client?.region ? regionMap[client.region] : '';
    }
    return '';
  };

  const getRegionName = (statusId) => {
    if (typeof statusId === 'string' && statusId.startsWith('client-')) {
      const clientId = statusId.includes('_type-')
        ? statusId.split('_type-')[0].replace('client-', '')
        : statusId.replace('client-', '');
  ...
  const getRegionClass = (regionName) => {
    const regionMap = {
      'South': 'south',
      'North': 'north',
      'Beirut': 'beirut',
  ...
  // Returns a fill color only when this client is shared with another
  // employee today (null otherwise, so the card just stays plain white) -
  // the whole card is colored, matching across every employee who has
  // that same client today, so the overlap is obvious at a glance.
  const getSharedClientColor = (statusId) => {
    const baseId = getBaseStatusId(statusId);

    if (typeof statusId === 'string' && statusId.startsWith('client-') && sharedClients && sharedClients.has(baseId)) {
      return generateClientColor(baseId);
    }
    return null;
  };
  ...
  return (
    <div
      ref={cellRef}
      className={`status-cell ${isTodayCell ? 'today' : ''} ${saving || isSavingState ? 'saving' : ''} ${isMobileHidden ? 'mobile-hidden' : ''}`}
      ...
      <div className="status-cell-wrapper">
        <div className="status-container">
          {Object.entries(groupedStatuses).map(([baseId, statusIds]) => {
              ...
              // Client name and type are kept separate (not one combined string)
              // so they render as their own rows in the card - name bold, type
              // as a quiet caption underneath - instead of one wrapped line.
              ...
              if (typeof baseId === 'string' && baseId.startsWith('client-')) {
                ...
                displayLabel = client?.name || 'Client';
                typeLabel = typeNames.length > 0 ? typeNames.join(' · ') : null;
              }

              const wrapperKey = statusIds.join('|');
              const regionInitials = getRegionInitials(statusIds[0]);
              const sharedColor = getSharedClientColor(statusIds[0]);
              ...
              return (
                ...
                >
                  {/* Status card - plain white, except when this client is
                      shared with another employee today, in which case the
                      whole card is filled with that client's color so the
                      overlap is obvious at a glance across employees. Region
                      tag and remove button live inside the card header
                      instead of floating outside it. */}
                  <div
                    className={`status-badge ${hasState ? `state-${state.state}` : ''}`}
                    style={{
                      backgroundColor: sharedColor || undefined,
                      opacity: isCancelled ? 0.6 : 1
                    }}
                    title={sharedColor ? 'Shared with another employee today' : undefined}
                  >
                    <div className="status-badge-header">
                      <span className="status-name">{displayLabel}</span>

                      {regionInitials && (
                        <span
                          className={`region-badge ${getRegionClass(getRegionName(statusIds[0]))}`}
                          title={`Region: ${getRegionName(statusIds[0])}`}
                        >
                          {regionInitials}
                        </span>
                      )}
                    ...
                    {typeLabel && (
                      <div className="status-type-subtitle">{typeLabel}</div>
                    )}

                    {/* State line - one line max, icon carries the color,
                        text stays a quiet neutral gray */}
                    {hasState && state.state === 'postponed' && (
                      <div className="status-state-line">
                        <span className="state-indicator postponed-indicator">{getStateIcon('postponed')}</span>
                        {state.isTBA ? (
                          <span>Postponed — TBA</span>
                        ) : state.postponedDate ? (
                          <span>Postponed from {formatDate(state.postponedDate)}</span>
                        ) : (
                          <span>Postponed — TBA</span>
                        )}
                      </div>
                    ...
                    {hasState && state.state === 'completed' && state.completedAt && (
                      <div className="status-state-line">
                        <span className="state-indicator completed-indicator">{getStateIcon('completed')}</span>
                        <span>Completed at {formatTimeOnly(state.completedAt)}</span>
                      </div>
                    )}

                    {hasState && state.state === 'cancelled' && (
                      <div className="status-state-line">
                        <span className="state-indicator cancelled-indicator">{getStateIcon('cancelled')}</span>
                        <span>Cancelled</span>
                        {hasCancellationReason && (
                          <button className="cancellation-info-btn" onClick={(e) => {
                            e.stopPropagation();
                            setShowCancellationDetails(baseId);
                          }}>
                            <svg width="12" height="12" viewBox="0 0 12 12">
                              <circle cx="6" cy="6" r="5" fill="#fee2e2" stroke="#dc2626" strokeWidth="1" />
                              <text x="6" y="8.5" textAnchor="middle" fill="#dc2626" fontSize="8" fontWeight="bold">i</text>
                            </svg>
                          </button>
                        )}
                      </div>
                    ...
                    <button
                      ...
                      style={{ opacity: hoverStatusId === statusIds[0] ? 0.6 : 0 }}
                      disabled={isSavingState}
                    >
                      ⋮
                    </button>

                    {/* Nested inside .status-badge (not a sibling in the day's
                        whole card stack) so it's positioned relative to THIS
                        specific card and opens beside it, not relative to the
                        whole column - which previously made it drop down
                        below every card in the day instead of next to the
                      ...
                      >
                        ...
                        {hasState && (
                          <div
                            ...
                            onClick={() => clearState(statusIds[0])}
                          >
          ...
          {/* Persistent "+ Add" ghost card - always part of the stack, not
              just a hidden click target on the empty cell area. Clicking it
              bubbles up to the cell's own onClick, which already opens the
              add-status dropdown - no separate handler needed here. */}
          {!saving && (
            <div className="add-status-ghost" title="Add status">
              <span className="add-status-ghost-icon">+</span>
              <span>Add</span>
            </div>
          )}
        </div>
          ...
          <div className="details-grid">
            ...
            <div className="detail-item">
              ...
              <div className="detail-value cancelled-date">
                {formatTimestamp(state.cancelledAt)}
              </div>
            </div>
          </div>
        </div>
