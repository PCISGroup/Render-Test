...
import { X, Mail, Users, Calendar, Send, CheckCircle, AlertCircle, Plus, Trash2, Wifi, WifiOff, Clock } from 'lucide-react';
...
export default function EmailSettingsModal({ settings, onSave, onClose }) {
  ...
  useEffect(() => {
    ...
    if (settings) {
      ...
      loadedSettings = {
        ...
        scheduledSendingEnabled: settings.scheduledSendingEnabled || settings.scheduled_sending_enabled || false,
        sendTime1: settings.sendTime1 || settings.send_time_1 || '',
        sendTime2: settings.sendTime2 || settings.send_time_2 || ''
      };
        ...
        try {
          ...
          if (parsed.recipients && Array.isArray(parsed.recipients)) {
            loadedSettings = {
              ...
              scheduledSendingEnabled: parsed.scheduledSendingEnabled || false,
              sendTime1: parsed.sendTime1 || '',
              sendTime2: parsed.sendTime2 || ''
            };
  ...
  useEffect(() => {
    if (!isOnline && formData.recipients.length > 0) {
      const settingsToCache = {
        ...
        scheduledSendingEnabled: formData.scheduledSendingEnabled,
        sendTime1: formData.sendTime1,
        sendTime2: formData.sendTime2,
  ...
  }, [formData.recipients, formData.includeWeekends, formData.scheduledSendingEnabled, formData.sendTime1, formData.sendTime2, isOnline, usingCachedSettings]);
  ...
  const handleSave = async () => {
    ...
    if (formData.scheduledSendingEnabled && !formData.sendTime1 && !formData.sendTime2) {
      showToast('error', 'Missing send time', 'Set at least one send time to enable automatic sending.');
      return;
    }
    ...
    try {
      ...
      const cleanFormData = {
        ...
        includeWeekends: formData.includeWeekends,
        scheduledSendingEnabled: formData.scheduledSendingEnabled,
        sendTime1: formData.sendTime1 || null,
        sendTime2: formData.sendTime2 || null
      };
          ...
          <div className="email-settings-section">
            ...
            <div className="email-preference-option">
              <label className="email-checkbox-container">
                <input
                  type="checkbox"
                  checked={formData.scheduledSendingEnabled}
                  onChange={(e) => setFormData({ ...formData, scheduledSendingEnabled: e.target.checked })}
                  disabled={isSaving}
                  className="email-checkbox-input"
                />
                <span className="email-checkmark"></span>
                <div className="email-checkbox-content">
                  <span className="email-checkbox-label">Send automatically, up to twice a day</span>
                  <span className="email-checkbox-desc">
                    Set one or both times below (Beirut time). The second time is optional.
                  </span>
                </div>
              </label>
            </div>

            {formData.scheduledSendingEnabled && (
              <div className="email-schedule-times">
                <div className="email-schedule-time-field">
                  <label htmlFor="send-time-1">First send time</label>
                  <input
                    id="send-time-1"
                    type="time"
                    value={formData.sendTime1}
                    onChange={(e) => setFormData({ ...formData, sendTime1: e.target.value })}
                    disabled={isSaving}
                    className="email-settings-input"
                  />
                </div>
                <div className="email-schedule-time-field">
                  <label htmlFor="send-time-2">Second send time (optional)</label>
                  <input
                    id="send-time-2"
                    type="time"
                    value={formData.sendTime2}
                    onChange={(e) => setFormData({ ...formData, sendTime2: e.target.value })}
                    disabled={isSaving}
                    className="email-settings-input"
                  />
                </div>
